...
class PresentationController {
    ...
    setupAutoResize() {
        const resizeSlide = () => {
            ...
            slide.style.minHeight = `${fullHeight}px`; // Ensure at least viewport height
            slide.style.height = 'auto';               // Allow taller slides to fit content
            ...
            if (typeof adjustListOverflow === 'function') {
                adjustListOverflow();
            }
        };
...
function markListLayouts() {
    document.querySelectorAll('.slide-content').forEach((section) => {
        if (section.querySelector('.content-list')) {
            section.classList.add('has-list');
        } else {
            section.classList.remove('has-list');
        }
    });
}

function adjustListOverflow() {
    window.requestAnimationFrame(() => {
        document.querySelectorAll('.slide-content.has-list').forEach((section) => {
            section.classList.remove('is-overflowing');

            section.querySelectorAll('.content-list').forEach((list) => {
                list.classList.remove('compact');
            });

            if (section.scrollHeight > section.clientHeight + 4) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.addEventListener('resize', adjustListOverflow, { passive: true });
    window.addEventListener('load', adjustListOverflow, { once: true });
